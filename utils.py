...
def plot_loss(checkpoint_dir, loss_list, save_pred_every):
    ...
    plt.xticks(range(0, len(loss_list)*save_pred_every+3,
                     (len(loss_list)*save_pred_every+10)//10))
    plt.legend()
    plt.grid()
...
def get_iou_v1(query_mask, pred_label, mode='foreground'):  # pytorch 1.0 version
    ...
    for i in range(num_img):
        ...
        union = torch.sum((combination == 1).float()).item() + \
            torch.sum((combination == 2).float()).item()
        if union != 0:
