...
class Dataset(object):
    ...
    def __init__(self, data_dir, fold, input_size=[321, 321], normalize_mean=[0, 0, 0],
        ...
        self.chosen_data_list = self.chosen_data_list_1 + \
            chosen_data_list_2+chosen_data_list_3
        self.chosen_data_list = self.chosen_data_list[:1000]

        # a dict of each class, which contains all imgs that include this class
        ...
        for index in range(1000):
            ...
            while True:  # random sample a support data
                ...
                if support_name != query_name:
                    break
            self.query_class_support_list[index] = [
                query_name, sample_class, support_name]
    ...
    def __getitem__(self, index):
        ...
        support_rgb = self.normalize(self.ToTensor(Image.open(
            os.path.join(self.data_dir, 'JPEGImages', support_name + '.jpg'))))
        support_mask = self.ToTensor(Image.open(os.path.join(
            self.data_dir, 'Binary_map_aug', 'val', str(sample_class), support_name + '.png')))
        query_rgb = self.normalize(self.ToTensor(Image.open(
            os.path.join(self.data_dir, 'JPEGImages', query_name + '.jpg'))))
        query_mask = self.ToTensor(Image.open(os.path.join(
            self.data_dir, 'Binary_map_aug', 'val', str(sample_class), query_name + '.png')))
